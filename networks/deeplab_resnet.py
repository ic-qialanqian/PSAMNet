...
class ResNet_locate(nn.Module):
    def __init__(self, block, layers):
        ...
        self.attention_pre = nn.Conv2d(2048, self.in_planes, 1, 1, bias=False)
        attention, infos = [], []
        for ii in [1, 3, 5]:    
            attention.append(nn.Sequential(nn.AdaptiveAvgPool2d(ii), AttentionLayer(self.in_planes, self.in_planes, kernel_size=7, padding=3, groups=1), nn.ReLU(inplace=True)))
        self.attention = nn.ModuleList(attention)

        self.attention_cat = nn.Sequential(nn.Conv2d(self.in_planes * 4, self.in_planes, 3, 1, 1, bias=False), nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        xs_1 = self.attention_pre(xs[-1])
        xls = [xs_1]
        for k in range(len(self.attention)):
            xls.append(F.interpolate(self.attention[k](xs_1), xs_1.size()[2:], mode='bilinear', align_corners=True))
        xls = self.attention_cat(torch.cat(xls, dim=1))
